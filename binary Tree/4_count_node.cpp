...
int count_node(Node* root){
    ...
    int r=count_node(root->rigt);
    return r+l+1;
}
int lifenode(Node* root){
    if(root==NULL)return 0;
    if(root->left==NULL&&root->rigt==NULL)return 1;
    int l=count_node(root->left);
    int r=count_node(root->rigt);
...
int max_hihgt(Node* root){
    if(root==NULL)return 0;
    if(root->left==NULL&&root->rigt==NULL) return 0;
    int l=max_hihgt(root->left);
    int r=max_hihgt(root->rigt);
    return (max(l,r)+1);
}
void count(Node* root){
    ...
    while(!q.empty()){
        Node* p=q.front();
        q.pop();
        if(p->left==NULL&&p->rigt==NULL) v.push_back(p->val);
        if(p->left)q.push(p->left);
        if(p->rigt)q.push(p->rigt);
    }
    cout<<v.size()<<nl;
